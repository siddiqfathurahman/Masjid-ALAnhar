...
<nav className="bg-hijau px-2 w-full z-10 font-poppins font-medium">
  <div className="container mx-auto flex items-center justify-between">
    <div className="text-white text-2xl font-bold">
      <img src="/logo.png" alt="Logo" className="lg:w-44 md:w-36 w-28" />
    </div>

 
    <ul className="hidden lg:flex items-center text-white space-x-6">
      <li className={`hover:text-lemon cursor-pointer ${isActive('/')}`}>
        <Link to="/" className="text-sm md:text-base lg:text-lg">Home</Link>
      </li>
      <li className={`hover:text-lemon cursor-pointer ${isActive('/profil')}`}>
        <Link to="/profil-masjid-alanhar" className="text-sm md:text-base lg:text-lg">Profil Masjid</Link>
      </li>
      <li className={`hover:text-lemon cursor-pointer ${isActive('/berita')}`}>
        <Link to="/berita" className="text-sm md:text-base lg:text-lg">Berita</Link>
      </li>
      ...
      <li className={`hover:text-lemon cursor-pointer ${isActive('/kontak')}`}>
        <Link to="/kontak" className="text-sm md:text-base lg:text-lg">Kontak</Link>
      </li>
      <li
        className={`cursor-pointer rounded-lg px-2 py-1 transition duration-200 bg-lemon text-hijau ${isActive('/al-quran')}`}
      >
        <Link to="/al-quran" className="text-sm md:text-base lg:text-lg">Al-Qur'an</Link>
      </li>
    </ul>
  ...
  <div
    className={`fixed top-0 right-0 h-full w-4/5 bg-white transform transition-transform duration-300 z-20 lg:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
  >
